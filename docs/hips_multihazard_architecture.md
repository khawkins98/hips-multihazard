...
#### Runtime additions to the in-memory dataset

The on-disk snapshot is the authoritative format above. After loading, `src/data/fetch-hips.js` augments the dataset with two fields used by the UI but never written to the bundled JSON:

- `data._source` — `"api"`, `"snapshot"`, `"cache"`, or `"stale-cache"`. Identifies which step of the fetch fallback chain produced this load.
- `data.meta.origin` — `"api"` or `"snapshot"`. Stamped before the data is written to the localStorage cache, so a later cache hit still knows whether the underlying payload came from a live API fetch or the bundled snapshot. Consumed by `src/ui/snapshot-info.js` to render the footer provenance label (`Live API` / `Snapshot` / `Cached · offline`) instead of the localStorage write time.
